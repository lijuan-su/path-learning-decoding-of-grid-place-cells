...
def create_weight(path,grids,places,flag):
    ...
    for j in range(0,int(len(weights)/3)):
        ...
        for i in [g_sum_maze, gp_sum_maze, p_sum_maze, yy]:
...
path1 = PathReward.CreatePath3(1000)
...
path4 = ExperimentPath.path_3rewards_Real()[10000:15000,1:3]
def plot_cell_path():
...
sim = abs(sim)
barplot.plt_bar(sim[:,0])
barplot.plt_bar(sim[:,1])
barplot.plt_bar(sim[:,2])

for i in plt.get_fignums():
    plt.figure(i)
    plt.savefig('figures/100_%s.pdf' % i)
