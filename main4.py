...
def create_weight(path,grids,places,flag):
    if flag == 1:
        weights = CreateWeight4.CreateWeight_modular(path,grids,places)
    else:
        weights = CreateWeight4.CreateWeight(path,grids,places)
...
# firingrate(path,grids1,places,'exprew path')
# firingrate(path,grids2,places,'exprew path')
print (similar)
...
for i in plt.get_fignums():
    plt.figure(i)
    plt.savefig('figures/SimRew15000_225_SP_%s.pdf' % i)
